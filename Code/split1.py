...
def organize_list_cars(zip, page_num):
    ...
    for i in new_list:
        ...
        seller_label.append(re.findall('"sellerDisplayLabel":"(.+)","streetAd', i ))
        rating.append(re.findall('"rating":(.+),"review',i))
        review_count.append(re.findall('"reviewCount":"(.+)","dealer',i))
        distance_from_zip.append(re.findall('"distanceFromSearchZip":"(.+)","customerId"',i))
    ...
    for i in range(len(make)):
        ...
        masterlist.append(str(price[i]) + str(make[i]) + str(model[i]) + str(year[i]) + str(bodyStyle[i]) + str(sellerRating[i])+str(city[i])+str(state[i])+str(mileage[i]) + str(seller_label[i]) + str(rating[i]) + str(review_count[i]) + str(distance_from_zip[i]) )
